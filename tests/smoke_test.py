...
def run() -> int:
    ...
    # byte 0x02 bit 6 = VIEW per SDL's Triton driver (was mis-labeled MENU
    # during early reverse engineering before the SDL source was located).
    sview = parser.parse(puck_frame2(b2=0x40))
    check(sview.pressed(Btn.VIEW), "byte 0x02 bit 6 -> VIEW")
    sqa = parser.parse(puck_frame2(b2=0x10))
    check(sqa.pressed(Btn.QUICK_ACCESS), "byte 0x02 bit 4 -> QUICK_ACCESS")
    srs = parser.parse(puck_frame2(b2=0x20))
    check(srs.pressed(Btn.RIGHT_STICK_CLICK), "byte 0x02 bit 5 -> R3 (RSCLICK)")
    sr4 = parser.parse(puck_frame2(b2=0x80))
    check(sr4.pressed(Btn.R4), "byte 0x02 bit 7 -> R4")
    ...
    sdd = parser.parse(puck_frame2(b3=0x04))
    check(sdd.pressed(Btn.DPAD_DOWN), "byte 0x03 bit 2 -> DPAD_DOWN")
    sdr = parser.parse(puck_frame2(b3=0x08))
    check(sdr.pressed(Btn.DPAD_RIGHT), "byte 0x03 bit 3 -> DPAD_RIGHT")
    sdl = parser.parse(puck_frame2(b3=0x10))
    check(sdl.pressed(Btn.DPAD_LEFT), "byte 0x03 bit 4 -> DPAD_LEFT")
    sdu = parser.parse(puck_frame2(b3=0x20))
    check(sdu.pressed(Btn.DPAD_UP), "byte 0x03 bit 5 -> DPAD_UP")
    smn = parser.parse(puck_frame2(b3=0x40))
    check(smn.pressed(Btn.MENU), "byte 0x03 bit 6 -> MENU")
    sls = parser.parse(puck_frame2(b3=0x80))
    check(sls.pressed(Btn.LEFT_STICK_CLICK), "byte 0x03 bit 7 -> L3 (LSCLICK)")
    sst = parser.parse(puck_frame2(b4=0x01))
    check(sst.pressed(Btn.STEAM), "byte 0x04 bit 0 -> STEAM")
    sl4 = parser.parse(puck_frame2(b4=0x02))
    check(sl4.pressed(Btn.L4), "byte 0x04 bit 1 -> L4")
    ...
    check(srtouch.pressed(Btn.RIGHT_STICK_TOUCH),
          "byte 0x04 bit 4 -> RIGHT_STICK_TOUCH (no longer false-mapped to RSCLICK)")
    check(not srtouch.pressed(Btn.RIGHT_STICK_CLICK),
          "right-stick TOUCH still does NOT trigger RSCLICK")
    sr2 = parser.parse(puck_frame2(b4=0x80))
    check(sr2.pressed(Btn.R2), "byte 0x04 bit 7 -> R2 digital")
    slt = parser.parse(puck_frame2(b5=0x01))
    check(slt.pressed(Btn.LEFT_STICK_TOUCH), "byte 0x05 bit 0 -> LEFT_STICK_TOUCH")
